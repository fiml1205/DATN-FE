...
import { listCity, listDistrict, listSport } from "@/utilities/constant"
import { validatePhone, validateEmail, validatePassword } from "@/utilities/functions";
import { loginApi, registerApi } from '@/app/api/authentication'
import LoadingAnimate from '@/components/loading-animate'

export default function Header({ SSToken, userInfor }: { SSToken: any; userInfor: any }) {
    console.log(SSToken)
    ...
    const [listSportRebuild, setListSportRebuild] = useState<any>()
    ...
    useEffect(() => {
        if (userInfor && userInfor.userName) {
            const nameParts = userInfor.userName.split(' ');
        ...
        const arrayListSportRebuild = listSport.map(item => ({
            value: item.sportID,
            label: item.sportName
        }))
        setListSportRebuild(arrayListSportRebuild)
    }, []);
    ...
    const selectSport = (sport: Number) => {
        setSport(sport)
    }
    ...
    const onSubmit = async (data: any) => {
        ...
        try {
            if (stateAuthen == 1) {
                const callApiLogin = await loginApi(data);
                console.log(callApiLogin);
            } else {
                data.type = typeUserAuthen == 1 ? 1 : 2;
                const callApiRegister = await registerApi(data);
                console.log(callApiRegister);
            }
        } catch (error: any) {
    ...
    const validateAccount = (value: string) => {
        ...
        return "Tài khoản phải là email hoặc số điện thoại hợp lệ";
    };
        ...
        <>
            <header className="flex justify-between px-3 xl:px-16 py-3">
                <div className="flex items-center gap-5">
                    <Link href='/'>
                        <Image src="/images/logo-site.svg" alt="logo-site" width={200} height={45} className="pt-3 mr-12" />
                    </Link>
                    <Link href='/blog' className="font-semibold text-default-color hidden xl:flex gap-1 items-center">
                        <BookOpenText />
                        Blog
                    </Link>
                    <Link href='/diamond-companies' className="font-semibold text-default-color hidden xl:flex gap-1 items-center">
                        <Gem />
                ...
                <div className="flex gap-2 items-center">
                    ...
                    {SSToken ? (
                        <div ref={avatarRef} className="md:flex p-2 gap-2 cursor-pointer items-center relative hidden" onClick={() => setShowAvatar(!showAvatar)}>
                            {userInfor?.avatar ? (
            ...
            <div className="bg-[url('/images/banner-home.webp')] w-full h-96 md:h-400px bg-cover bg-no-repeat bg-bottom px-5 py-12 xl:py-20 flex items-center flex-col gap-5">
                ...
                <div className="w-full max-w-xl flex flex-col gap-4 lg:gap-0 lg:flex-row">
                    ...
                    <Combobox listData={listSportRebuild} placeholder={'thể loại'} borderRadius={1} handleFunction={selectSport} />
            ...
            {showPopupAuthen ?
                <div className="layout">
                    <div className="max-w-400px h-fit relative flex flex-col justify-center items-center gap-3 bg-white rounded-md w-full pb-6 pt-4 bg-color-dark shadow-color-dark border-dark">
